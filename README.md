...
├── prompt-template.js  AIへの指示文（プロンプト）。単独ファイルなので直接編集・確認しやすい
...
Anthropic（Claude）とGoogle（Gemini）のどちらかを選んで使えます。Geminiには
無料枠があります。

1. **Anthropicを使う場合**：https://console.anthropic.com でAPIキーを発行
2. **Geminiを使う場合**：https://aistudio.google.com/apikey でAPIキーを発行（無料）
3. アプリ右上の⚙アイコンを開き、「使用するAI」でどちらかを選択 → APIキーとモデル名を入力して保存

**注意**：APIキーはあなたのブラウザの中だけに保存され、選んだAIサービス以外には
送信されません。ただし、ブラウザの開発者ツールを見ればキーの文字列自体は
見えてしまう仕組みです。他の人と共有する端末では使わないことをおすすめします。

### AIへの指示文（プロンプト）をカスタマイズする

日記をどう書かせるかの指示文は `prompt-template.js` に書かれています。
このファイルを直接編集してもよいですし、アプリを開いたまま⚙設定の
「AIへの指示文（プロンプト）」欄から、コードを触らずに直接編集することもできます
（こちらの方が手軽です）。`{{mood}}` `{{tags}}` `{{memo}}` の3つはその日の内容に
自動で置き換わるプレースホルダーなので、削除しないでください。
